...
try:
    ...
    print(
        "Warning: 'trimesh' library could not be imported. Mesh repair functionality will be skipped."
    )
    trimesh = None
...
def convert_stl_to_mesh(stl_path, output_path="molar_mesh.xdmf", solidity_check=True):
    ...
    if trimesh is not None and solidity_check:
        try:
            ...
            try:
                ...
                cleaned = (
                    pv_mesh.clean(
                        merging_points=True, tolerance=1e-3, remove_unused_points=True
                    )
                    .fill_holes(100)
                    .triangulate()
                )
                ...
                print(
                    f"  > PyVista/VTK specific repair failed ({inner_e}). falling back to Trimesh results."
                )
                temp_stl = "temp_repaired.stl"
    ...
    try:
        ...
        print(
            "  - MeshLab: Filters -> Cleaning and Repairing -> Remove Duplicated Faces"
        )
        print(
            "  - Blender: Edit Mode -> Select All -> Mesh -> Clean Up -> Merge by Distance"
        )
        print("-" * 60 + "\n")
        gmsh.finalize()
