...
def export_to_glb(xdmf_path, output_glb="molar_mesh_view.glb"):
    ...
    try:
        ...
        mesh = trimesh.Trimesh(vertices=m.points, faces=m.cells_dict.get("tetra")) # tetra를 직접 쓰면 내부까지 포함되지만 뷰어에서는 표면만 보임
        
        # 외부 표면만 추출 (더 깔끔한 뷰를 위해)
        surface_mesh = mesh.section(plane_origin=mesh.centroid, plane_normal=[1,0,0]).to_mesh() if False else mesh
        
        # GLB로 저장 (Windows 3D 뷰어 호환)
        surface_mesh.export(output_glb)
        print(f"Successfully exported to {output_glb}. You can open this with Windows 3D Viewer.")
